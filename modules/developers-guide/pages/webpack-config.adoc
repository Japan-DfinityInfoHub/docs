...
* React dapp構築のチュートリアル link:tutorials/custom-frontend{outfilesuffix}[フロントエンドをカスタマイズする］
* link:tutorials/hello-location{outfilesuffix}#candid-ui[Candid]を、キャニスター内の機能を公開してテストするための最低限のインターフェースとして使用する。
* link:tutorials/explore-templates{outfilesuffix}#default-frontend[raw HTML and JavaScript]を使って、シンプルなHTMLエントリーページを表示しています。
* link:tutorials/custom-frontend{outfilesuffix}[ReactとコンパイルされたJavaScript]を使って、HTMLの属性や要素を直接ページに埋め込みます。
* link:tutorials/my-contacts{outfilesuffix}[ReactとTypeScript]を使って、外部ファイルからCSSプロパティをインポートする。
...
webpack以外のbundlerを使いたい場合もあるでしょう。バンドルラーごとの説明はまだ準備できていませんが、お使いのバンドラーに精通していれば、以下の手順で作業を進めることができます。
...
. declarations/<canister_name>/index.js+` を修正し、`+process.env.<CANISTER_NAME>_CANISTER_ID+` をバンドルラーの環境変数に相当するパターンに置き換えます。
